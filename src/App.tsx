...
function App() {
  ...
  const buttonPress = (symbol: string) => {
    if (symbol === "clear") {
      ...
      // dont forget to split by OPERATORS and get last # 
      const lastNumber = expression.split(/[-+/*]/g).pop();
      // REMINDER: if last # already contains decimal, dont add another
  ...
  const calculate = () => {
    // if last chart is an operator, do nothing :
    if (isOperator(et.charAt(et.length - 2))) return;

    // clean expression so 2 operators in a row use the last operator ??
    const parts = et.split(" ");
    const newParts = [];

    // loop so that we go through parts backwards 
    for (let i = parts.length - 1; i >= 0; i--) {
      if (["*", "/", "+"].includes(parts[i]) && isOperator(parts[i - 1])) {
        newParts.unshift(parts[i - 1]);
        let j = 0;
        let k = i - 1;
